...
import Student from "../models/Student.js"; // Explicit Student Model
...
router.post("/register", async (req, res) => {
  try {
    console.log("Incoming Register Request Body:", req.body); // Debugging
...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id, name: user.name, email: user.email, role: user.role }, "your_secret_key", { expiresIn: "1h" });
...
router.get("/teachers", checkAdmin, async (req, res) => {
  try {
    console.log("Fetching Teachers - User:", req.user); // ✅ Log user
...
router.post("/teachers/create", checkAdmin, async (req, res) => {
  console.log("Creating Teacher - Request by:", req.user); // ✅ Log user
...
router.put("/teachers/update/:id", checkAdmin, async (req, res) => {
  try {
...
router.delete("/teachers/delete/:id", checkAdmin, async (req, res) => {
  try {
...
// GET All Students (Admin Only) – populate student profile and class details.
...
router.post("/students/create", checkAdmin, async (req, res) => {
  try {
    ...
    // Look up the Class by its unique combination.
    ...
    // Create the Student document with reg_id (converted to number) and class reference.
    const newStudent = new Student({ reg_id: Number(reg_id), class: classExists._id });
    console.log("New Student Document before save:", newStudent);
    await newStudent.save();

    // Create the corresponding User document with role "student"
...
router.put("/students/update/:id", checkAdmin, async (req, res) => {
  try {
    console.log("Incoming Update Request Body:", req.body);
    const { name, email, password, profile } = req.body;
    // From profile, we expect: reg_id, class_code, and commencement_year.
    ...
    // Fetch the existing student with populated profile.
    ...
    // Update User fields (name, email, optionally password)
    ...
    // Update the Student profile with reg_id (converted to number) and new class reference.
    ...
    // Fetch the final updated student with populated profile and class.
...
router.delete("/students/delete/:id", checkAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
